...
where calon.divisi ='$_POST[session]' and calon.id not in (select calon_id from hasil_seleksi where persetujuan = 'Disetujui')");
...
while ($data_row = mysqli_fetch_assoc($hasil)) {
	...
	WHERE divisi ='$_POST[session]' and calon.id not in (select calon_id from hasil_seleksi where persetujuan = 'Disetujui') 
	...
	WHERE divisi ='$_POST[session]' and calon.id not in (select calon_id from hasil_seleksi where persetujuan = 'Disetujui') and CALON_SUBKRITERIA.CALON_ID =" . $datas['data'][$data_row['id']]['id']);
	} else {
		$hasil2 = mysqli_query($koneksi, "SELECT KRITERIA.nama, KRITERIA.id, SUBKRITERIA.NAMA AS nama_subkriteria, 
	SUBKRITERIA.BOBOT AS bobot_subkriteria, CALON_SUBKRITERIA.value 
	FROM CALON_SUBKRITERIA JOIN SUBKRITERIA ON (SUBKRITERIA.ID=CALON_SUBKRITERIA.subkriteria_id) 
